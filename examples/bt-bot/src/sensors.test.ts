...
function createClientFake(initialUsers: User[]): {
...
test('キルスイッチは表示名ではなくユーザーIDで運営を認可する', () => {
  ...
  assert.ok(logs.some((message) => message.includes('OPERATOR_IDS')))
})

test('self判定はsession IDを優先し、同名の別ユーザーを除外しない', () => {
  const self: User = { id: 'self-id', name: 'Shared Name', isBot: true }
