...
- **ツール呼び出し対応**: 「今何時？」と聞くと現在時刻を教えてくれます
- **ミュート機能**: 「黙って」と言うと静かになり、「話して」と言うと再開します
- `q` - 終了（Ctrl+Cでも可）
...
- **時刻の確認**: 「今何時？」「現在時刻は？」などと質問すると、現在の日本時間を音声で回答します
- **ミュート制御**: 
  - 「黙って」「静かにして」「うるさい」→ ボットが黙ります
  - 「話して」「しゃべって」「ミュート解除」→ 再び話すようになります
...
- Gemini Live API (gemini-live-2.5-flash-preview) との通信
...
- **Geminiモデル**: gemini-live-2.5-flash-preview
